...
export const startKb = () => Markup.keyboard(['🔑 Начать'], { resize: true }).oneTime().resize();

export const mainKb = () =>
  Markup.keyboard([['📸 Создать альбом', '📚 Мои альбомы'], ['✅ Опубликовать всё']]).resize();
...
export function viewKeyboard(userId, albumId, idx, len) {
  ...
  if (idx < len - 1)
    row.push(Markup.button.callback('➡️ Далее', `${prefix}:${ACTION.next}:${idx}`));
...
export const finishEditKb = () => Markup.keyboard(['✅ Завершить редактирование']).resize();
